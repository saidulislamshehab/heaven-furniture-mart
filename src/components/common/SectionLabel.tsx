...
export function SectionLabel({ number, children, className, ...props }: SectionLabelProps) {
  return (
    <p className={cn('eyebrow inline-flex items-center gap-3 text-eyebrow-gold', className)} {...props}>
      {number && <span className="font-mono text-[0.7rem] font-normal tracking-[0.1em]">{number}</span>}
